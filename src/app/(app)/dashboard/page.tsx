...
import { acceptingMessagesSchemaValidation } from "@/schemas/acceptingMessagesSchema";
...
const DashboardPage = () => {
  ...
  const handleDeleteMessage = (deletedMessage: IMessage) => {
  ...
  const { data: session } = useSession();
   const form = useForm({
      resolver: zodResolver(acceptingMessagesSchemaValidation),
    });
    ...
    try {
      ...
      setValue('isAcceptingMessages', acceptingMessagesSchemaValidation.parse(response.data).isAcceptingMessages);
    } catch (error) {
  ...
  const fetchMessages = useCallback(
    async (refresh: boolean = false) => {
      ...
      try {
        ...
        if (refresh) {
          toast.loading('Messages refreshed', { duration: 2000 });
        }
      } catch (error) {
        const axiosError = error as AxiosError<ApiResponse>;
        toast.error(
